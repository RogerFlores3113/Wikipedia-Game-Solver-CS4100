...
for i, metric in enumerate(['Time Taken']):
    axes.bar(np.arange(len(approaches)) - 0.2, np.mean(time_taken_data, axis=0),
                width=0.2, label='Time Taken', align='center')
    axes.bar(np.arange(len(approaches)), np.mean(priority_queue_data, axis=0),
                width=0.2, label='Priority Queue Size', align='center')
    axes.bar(np.arange(len(approaches)) + 0.2, np.mean(accuracy_data, axis=0),
